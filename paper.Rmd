...
data %>%
    ...
    mutate(time.to.death=difftime(death.date, aids.date, units="weeks") / 4) %>%
    group_by(frst.aids.diag) %>%
    summarise(cases=n(),
              med.to.death=median(time.to.death))
...
visits <- read_csv("data/hivvert-data04.csv", progress=FALSE,
    ...
    filter(COHORT == 1) %>% 
    group_by(STUDYID) %>%
    summarise(drug.use=sum(CRACKUSE, COCUSE, HERUSE, AMPHUSE, na.rm=TRUE) > 1,
              depression=mean(CESD, na.rm=TRUE) < 23) %>%
    ungroup() %>%
...
Finally, we use propensity score matching to account for confounding bias.

```{r propensity-score}
caliper <- 0.07
set.seed(90210)
match.set <- MatchIt::matchit(sec.aids ~ factor(race) + 
                              age.at.aids + drug.use +
                              aids.year + depression + cd4,
                              data=time.varying,
                              distance="logit",
                              method="nearest",
...
# ## let's check out the diagnostics

# ## numerical check
match.check <- summary(match.set, standardize=TRUE)

# ## graphical check
plot(match.set, type="hist")
# plot(match.set, type="jitter", interactive=FALSE)

matched.dat <- MatchIt::match.data(match.set,"all")

ph.match <- coxph(Surv(start, stop, status) ~ sec.aids +
                  factor(race) + age.at.aids + drug.use +
                  aids.year + depression + cd4,
   				  data=matched.dat,
                  method="efron")
...
There were `r length(unique(matched.dat$STUDYID))` individuals in the 
matched data set of the orginal `r nrow(data)`. There are 
`r sum(matched.dat$sec.aids)` events in the matched data set.
We perform a check on the data and see the following summaries. We used a
caliper of `r caliper`.

```{r}
knitr::kable(match.check$sum.all[,1:4],
             caption="Summary of balance for all data")
knitr::kable(match.check$sum.matched[,1:4],
             caption="Summary of balance for matched data")
knitr::kable(match.check$reduction,
             caption="Percent Balance Improvement")
knitr::kable(match.check$nn,
             caption="Sample Sizes")
```

In recalculating our proportional hazards model using propensity score
in order to handle confounding bias, we see the following results.

```{r}
knitr::kable(coef(summary(ph.match)),
             caption="PH Model (Propensity Scores)")

knitr::kable(cox.zph(ph.match)$table,
             caption="PH Assumption Test")
caliper <- 0.5
```

Next, we performed the same PH model using propensity scores, but with a higher
caliper (`r caliper`).
...
There were `r length(unique(matched.dat$STUDYID))` individuals in the 
new matched data set of the orginal `r nrow(data)`. There are 
`r sum(matched.dat$sec.aids)` events in the matched data set.
We perform a check on the data and see the following summaries. 
...
In our second PH model with propensity scores, we see the following results.
...
Finally, we used a subclassification method for our propensity score.

```{r subclass}
sub.set <- MatchIt::matchit(sec.aids ~ factor(race) + 
                            age.at.aids + drug.use +
                            aids.year + depression + cd4,
                            data=time.varying,
                            method="subclass",
...
# summary(sub.set,standardize=T)
plot(sub.set,type="jitter",interactive=F)
# plot(summary(sub.set,standardize=T),interactive=F)

sub.dat <- MatchIt::match.data(sub.set, "all")
ph.match <- coxph(Surv(start, stop, status) ~ sec.aids + strata(subclass),
   				  data=sub.dat,
                  method="efron")
knitr::kable(coef(summary(ph.match)),
             caption="PH Model (Propensity Scores)")
...
```{r graphs}
plot(survfit(ph.match), main="Final PH Model Using Subclassification")
```
...
Keri's recommendations:
    ...
    7. CD4 is great
    8. year of aids diagnosis? restrict to 1996. Censor study at 1996. could
       categorize year (<90, 90-93, 93-Jul 95, >Jul 95)
    9. put people back in that had sec aids at baseline.
    10. timing of secondary aids diagnosis? distribution of prim aids to sec. aids
        infection?
    11. sens. look at dist of survival by type. pick a cutoff that we think is
        meaningful. type of primary. i.e. higher median survival, lower median
        survival.
    12. switch propensity scores to ipw
    13. combinations of prim and sec aids impact. could explore with
        interaction.
    14. Add site
